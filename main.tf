...
resource "authentik_stage_identification" "ldap_generic" {
  ...
  password_stage = authentik_stage_password.ldap_generic.id
}
...
resource "authentik_flow_stage_binding" "ldap_generic_identification" {
  count  = local.enable_generic_ldap_provider ? 1 : 0
  stage  = authentik_stage_identification.ldap_generic.id
  target = authentik_flow.ldap_generic.uuid
  order  = 10
}

resource "authentik_flow_stage_binding" "ldap_generic_login" {
  count  = local.enable_generic_ldap_provider ? 1 : 0
  stage  = authentik_stage_user_login.ldap_generic.id
  target = authentik_flow.ldap_generic.uuid
  order  = 30
}
...
resource "authentik_provider_ldap" "generic" {
  ...
  bind_flow = authentik_flow.ldap_generic.uuid
}
...
resource "authentik_application" "ldap_generic" {
  ...
  name             = "ldap-generic"
  slug             = "ldap-generic"
  protocol_provider = authentik_provider_ldap.generic.id
}
...
resource "authentik_outpost" "ldap_generic" {
  ...
  name               = "ldap-generic"
  type               = "ldap"
  protocol_providers = [authentik_provider_ldap.generic.id]
}
